...
public class CustomerPhoneService {
	...
	public List<String> getAllPhones() {
		...
		RegisteredCustomers.getRegisterdCustomers().forEach((customerId, customer) -> {
			customer.getPhones().stream().forEach(phone -> phoneNumberList.add(phone.getPhoneNumber()));
		});
	...
	public List<String> getPhoneForACustomer(String customerId) {
		...
		if (matchingCustomer.isPresent()) {
			matchingCustomer.get().getPhones().forEach(phone -> phoneList.add(phone.getPhoneNumber()));
		}
		return phoneList;
	}

	public void activatPhone(String customerName, String customerId, String phoneNumber, String phoneType) {
